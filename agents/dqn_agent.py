...
into the Agent-2048 InteractionModule and RunLogger.
    ...
    │ Agent           │ Details                                      │
    ├─────────────────┼──────────────────────────────────────────────┤
    │ DQNAgent4x4     │ achraf-azize architecture                    │
    │                 │ Parallel 1×2 / 2×1 convolutions, 2 levels   │
    │                 │ Trained with nb_empty_tiles reward           │
    │                 │ Checkpoint key: 'online'                     │
    │                 │ Trained: 3 000 epochs (~3 hrs GPU)           │
    │                 │ 2048 rate: ~3–5%                             │
    ├─────────────────┼──────────────────────────────────────────────┤
    │ DQNAgent6x6     │ ResNet trunk architecture                    │
...
# Action order used during training in play2048.py (4×4 model):
...
# 6×6 model ACTIONS = [UP, DOWN, LEFT, RIGHT] matches Action.value directly
_6X6_ACTIONS = [Action.UP, Action.DOWN, Action.LEFT, Action.RIGHT]

MAX_TILE_POWER = 17   # used by 6×6 one-hot encoder


# ═══════════════════════════════════════════════════════════════════
#  4×4 STATE ENCODING — achraf-azize / play2048.py
# ═══════════════════════════════════════════════════════════════════

def _encode_4x4(board: np.ndarray) -> torch.Tensor:
    """(4, 4) int → (1, 16, 4, 4) float tensor.
...
# ═══════════════════════════════════════════════════════════════════
#  6×6 STATE ENCODING — identical to dqn_agent.py (6×6 branch)
# ═══════════════════════════════════════════════════════════════════

def _encode_6x6(board: np.ndarray) -> np.ndarray:
    """(H, W) int → (MAX_TILE_POWER+1, H, W) float32 ndarray.

    Identical to encode_board() in the 6×6 dqn_agent.py so the
...
# ═══════════════════════════════════════════════════════════════════
#  4×4 NETWORK — achraf-azize architecture
# ═══════════════════════════════════════════════════════════════════

class _DQN4x4(nn.Module):
    """Parallel 1×2 / 2×1 convolutions, two levels deep."""
    def __init__(self):
        super().__init__()
...
# ═══════════════════════════════════════════════════════════════════
#  6×6 NETWORK — ResNet trunk architecture
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
#  DQNAgent4x4 — BaseAgent wrapper for the 4×4 model
# ═══════════════════════════════════════════════════════════════════

class DQNAgent4x4(BaseAgent):
    """DQN agent using the achraf-azize architecture (4×4 board).
    ...
    def get_params(self) -> Dict:
        return {
            "architecture":   "achraf-azize (parallel 1×2 / 2×1 conv)",
            "grid_size":      4,
...
# ═══════════════════════════════════════════════════════════════════
#  DQNAgent6x6 — BaseAgent wrapper for the 6×6 model
# ═══════════════════════════════════════════════════════════════════

class DQNAgent6x6(BaseAgent):
    """DQN agent using the ResNet trunk architecture (6×6 board).
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate DQN agents (4×4 or 6×6) and write log files.")
    parser.add_argument("--model",   required=True,
                        help="Path to trained .pt checkpoint")
    parser.add_argument("--grid",    type=int, default=4, choices=[4, 6],
                        help="Board size: 4 for 4×4 model, 6 for 6×6 model")
